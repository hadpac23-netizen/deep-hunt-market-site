...
for(let i=0;i<5;i++)rows.push({provider:"secondary",success:i<4,status_code:i<4?200:500});
...
assert.equal(state.secondary.open,false,"healthy secondary circuit opened");
...
assert.equal(state.secondary.success_rate,.8,"secondary success rate wrong");
...
console.log(JSON.stringify({openai:state.openai,secondary:state.secondary,gemini:state.gemini,tiny:state.tiny},null,2));
