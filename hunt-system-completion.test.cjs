...
assert(analytics.includes("if(!consentGranted)return false"),"experience analytics must honor consent");
...
assert(read("legal-config.json").includes('"payments_status": "PRELAUNCH"'),"legal payment status must remain PRELAUNCH");
