(() => {
  ...
  const common = {
    ...
    ja: {
      language: "言語", live: "ライブ", loading: "読み込み中…", verified: "確認済み",
      affiliateDisclosure: "開示: 対象リンクからの購入によりHUNTが追加費用なしで手数料を受け取る場合があります。",
      nasaCredit: "Earth imagery: NASA / Earth Observatory."
    },
    zh: {
      language: "语言", live: "实时", loading: "加载中…", verified: "已验证",
      affiliateDisclosure: "披露：通过符合条件的链接购买时，HUNT可能获得佣金，用户无需支付额外费用。",
      nasaCredit: "Earth imagery: NASA / Earth Observatory."
    }
  };
  ...
  function apply(page, lang) {
    const supported = new Set(["en","ar","he","es","fr","ja","zh"]);
    const safe = supported.has(lang) ? lang : "en";
    const dict = {...(common[safe] || common.en), ...(pages[page]?.[safe] || pages[page]?.en || {})};
