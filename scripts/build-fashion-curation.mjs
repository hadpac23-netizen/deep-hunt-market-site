...
function usable(item){
  ...
  return Boolean(item?.item_id && item?.provider && title && httpsImage(item?.image_url) && positivePrice(item?.price_amount)
    && !blocked.test(title) && !sexualized.test(title) && !bodyIdeal.test(title) && !kids.test(title));
}
function nonFashionNoise(title){ return obviousNonFashion.test(title); }
function womenText(t){ return /\b(women(?:'s)?|woman|female|ladies)\b/i.test(t); }
function menText(t){ return /\b(men(?:'s)?|man|male|gentlemen)\b/i.test(t); }
...
const tests = {
  ...
  shoes: t => !nonFashionNoise(t)
    && /\b(shoes?|sneakers?|loafers?|heels?|pumps|boots?|sandals?|slippers?|mules?|oxfords?|ballet flats?|flat shoes?)\b/i.test(t)
    && !/\b(shoe cover|shoes cover|shoe rack|shoe storage|shoe repair|repairing|shoe tree)\b/i.test(t),
...
for(const item of all.values()){
  ...
  for(const [slug,test] of Object.entries(tests)){
    if(test(title)) curated[slug].push({...item,source_category:item.category||null,category:slug,catalog_discovery:true,availability_verified:item.availability_verified===true});
  }
}
...
curated.women=union([...apparelSlugs,...accessorySlugs]).filter(x=>womenText(x.title) && !menText(x.title));
curated.men=union([...apparelSlugs,...accessorySlugs]).filter(x=>menText(x.title) && !womenText(x.title));
...
const homeSlugs=["women","dresses","tops","activewear","jackets","knitwear","shoes","bags","jewelry","accessories","hats","men","suits","underwear","socks"];
const homeShelves={};
...
for(const slug of ["women","dresses","tops","bottoms","hoodies","jackets","knitwear","activewear","suits","underwear","socks","shoes","bags","jewelry","accessories","hats","men"]){
