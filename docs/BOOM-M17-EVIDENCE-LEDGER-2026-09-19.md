...
Marketplace snapshot can now become VERIFIED through M19 when all required read-only marketplace counts succeed. M20 now exposes paid-attribution readiness explicitly. M22 supplies read-only server purchase proof and M23 supplies durable-event-identity readiness, but paid_attribution remains MISSING until the full live chain including event-id persistence, database uniqueness and campaign linkage is proven.
