(() => {
  ...
  async function applySession(next){
    const sameUser=String(session?.user?.id||"")===String(next?.user?.id||"");
    session=next||null;
    if(session?.user){
      if(!sameUser || !lastServer) await hydrate();
    }else{
      lastServer=null;
      renderPreferenceUI();
  ...
  async function init(){
    renderPreferenceUI();
    client=runtime?.getSupabaseClient?.() || window.HuntAccountClient || null;
    if(!client){
      window.dispatchEvent(new CustomEvent("hunt:personalization-ready",{detail:{signedIn:false,localOnly:true}}));
      return;
    }
    ...
    if(runtime?.sessionReady){
      await applySession(await runtime.sessionReady());
    }else{
    ...
    if(runtime?.subscribeSession){
      runtime.subscribeSession(next=>{ void applySession(next||null); });
    }else{
      client.auth.onAuthStateChange((_event,next)=>{ void applySession(next||null); });
    }
  }
