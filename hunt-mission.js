(() => {
  ...
  const quantity=document.querySelector("#hd-mission-quantity");
  ...
  try{
    ...
    if(saved){ query.value=saved.query||""; quantity.value=saved.quantity||1; budget.value=saved.budget||""; deadline.value=saved.deadline||""; }
  }catch{}
  form.addEventListener("submit",event=>{
    event.preventDefault();
    const q=String(query.value||"").trim().slice(0,120);
    if(!q)return;
    const qty=Math.max(1,Math.min(20,Number(quantity.value||1)||1));
    const b=Number(budget.value||0);
    const mission={query:q,quantity:qty,budget:Number.isFinite(b)&&b>0?b:null,deadline:String(deadline.value||"")||null,created_at:new Date().toISOString()};
