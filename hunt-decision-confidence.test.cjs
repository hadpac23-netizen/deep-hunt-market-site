...
for(const token of ["renderDecisionCheck","Destination recheck","selectedAvailability","cartReady","Quote verified","Needs check"]){
...
assert(sw.includes('hunt-shell-pwa8'),"PWA cache version not bumped");
...
assert(pwa.includes("service-worker.js?v=pwa8"),"PWA registration version mismatch");
