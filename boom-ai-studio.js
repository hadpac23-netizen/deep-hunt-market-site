(() => {
  ...
  function buildConnectRows(){
    ...
    return [
      ...
      {id:"posthog",name:"PostHog EU",kind:"ANALYTICS + EVIDENCE",tone:"healthy",detail:"Product Analytics and canonical Web Analytics $pageview ingestion are verified in BOOM Studio EU. Session Replay SDK runtime is verified locally; server-side replay evidence still requires the protected Preview diagnostic. Error Tracking is enabled but remains unverified until a real $exception issue is observed.",evidence:"Product + $pageview VERIFIED · Replay SDK RUNTIME VERIFIED · Error/Replay server evidence pending",gate:"Owner gate for project/token or privacy-policy changes"},
