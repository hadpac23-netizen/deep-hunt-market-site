(() => {
  ...
  function renderProfessionalWorkbench(){
    ...
    if(safetyReport)safetyReport.textContent=[
      ...
      "RED TEAM COMPLETED/LINKED: "+snapshot.safety.redteam.completed_linked,
      "RED TEAM COVERAGE: "+snapshot.safety.redteam.covered_cases+"/"+snapshot.safety.redteam.cases,
      "RED TEAM PENDING: "+snapshot.safety.redteam.pending,
      "RED TEAM UNLINKED: "+snapshot.safety.redteam.unlinked,
      "RED TEAM FAILURES: "+snapshot.safety.redteam.failed,
      "CONFIDENCE CALIBRATION: valid="+snapshot.safety.calibration.valid_rows+"/"+snapshot.safety.calibration.rows+" invalid="+snapshot.safety.calibration.invalid_rows,
    ...
    report.textContent=[
      ...
      "RED TEAM: covered="+snapshot.safety.redteam.covered_cases+"/"+snapshot.safety.redteam.cases+" completed_linked="+snapshot.safety.redteam.completed_linked+" pending="+snapshot.safety.redteam.pending+" unlinked="+snapshot.safety.redteam.unlinked+" failed="+snapshot.safety.redteam.failed,
      "CONFIDENCE CALIBRATION: valid="+snapshot.safety.calibration.valid_rows+"/"+snapshot.safety.calibration.rows+" invalid="+snapshot.safety.calibration.invalid_rows+" max_error="+String(snapshot.safety.calibration.max_error??"unmeasured"),
      "MULTI-AGENT JUDGE: completed="+snapshot.safety.team_judge.judged+"/"+snapshot.safety.team_judge.runs+" incomplete="+snapshot.safety.team_judge.incomplete,
  ...
  const promptCoverageItems=Object.freeze([
    ...
    {id:"real-device",label:"Viewport visual QA · 390 / 768 / 1280 / 1440",status:"PRESENT",evidence:"Fresh branch browser QA passed 4/4 after the safe-preview status fix: 390 / 768 / 1280 / 1440 all report pageOverflow=0, workspaceOverflow=0 and Safe Preview active."},
  ...
  function buildOwnerAlphaEvidencePack(){
    ...
    if(!EvidencePack?.build||!EvidencePack?.verifyIntegrity||!AlphaHarness?.runAll){
    ...
    const integrityCheck=EvidencePack.verifyIntegrity(pack);
    const valid=integrityCheck.valid===true;
    ...
    if(status)status.textContent=!valid
      ?"BLOCKED_EVIDENCE_INTEGRITY · PRODUCTION_OFF"
    ...
    if(report)report.textContent=[
      ...
      "INTEGRITY CHECK: "+(valid?"PASS":"FAIL"),
      "INTEGRITY ISSUES: "+(integrityCheck.issues.join(", ")||"none"),
