(() => {
  ...
  const VERSION="2026-09-19-f50-r1";
  ...
  function inspect(candidate={},evidenceEngine){
    const blockers=[];
    ...
    const mechanism=clean(candidate.mechanism,800);
    const payer=clean(candidate.payer,300);
    const primitive=clean(candidate.economic_primitive,80).toUpperCase();
    const scale=clean(candidate.scale_math,800);
    const priorArt=clean(candidate.prior_art_conclusion,800);
    const redTeam=Array.isArray(candidate.red_team)?candidate.red_team.filter(Boolean):[];
    const copyRisk=clean(candidate.big_tech_copy_risk,40).toUpperCase();
    const novelty=clean(candidate.novelty_claim,800);
    const moat=clean(candidate.moat,800);
    ...
    if(!priorArt)blockers.push("prior_art_conclusion_missing");
    if(!novelty)blockers.push("novelty_claim_missing");
    if(!moat)blockers.push("moat_missing");
    if(redTeam.length<3)blockers.push("red_team_incomplete");
    ...
    if(candidate.mechanism_novelty===false)blockers.push("mechanism_not_novel");
    ...
    const evidence=evidenceEngine?.inspectCandidate
      ...
      : {evidence_ready:false,blockers:["evidence_engine_unavailable"],usable_count:0,independent_count:0,missing_novelty_surfaces:[]};
  ...
  function killOrKeep(candidate={},evidenceEngine){
    const result=inspect(candidate,evidenceEngine);
    return Object.freeze({
      ...result,
