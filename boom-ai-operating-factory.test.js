...
assert.equal(skills.skills.length,10);
assert.equal(templates.templates.length,10);
assert.equal(new Set(skills.skills.map(x=>x.id)).size,10);
assert.equal(new Set(templates.templates.map(x=>x.id)).size,10);
...
console.log("BOOM AI operating factory: PASS — 10 skills, 10 templates, canonical owners, material Owner Gates, plan-only router");