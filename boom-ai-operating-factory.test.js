...
assert.equal(templates.templates.length,13);
assert.equal(new Set(skills.skills.map(x=>x.id)).size,13);
assert.equal(new Set(templates.templates.map(x=>x.id)).size,13);
...
console.log("BOOM AI operating factory: PASS — 13 skills, 13 templates, canonical owners, material Owner Gates, plan-only router");