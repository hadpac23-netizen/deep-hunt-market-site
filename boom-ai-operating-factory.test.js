...
assert.equal(new Set(skills.skills.map(x=>x.id)).size,8);
assert.equal(new Set(templates.templates.map(x=>x.id)).size,8);
...
console.log("BOOM AI operating factory: PASS — 8 skills, 8 templates, canonical owners, material Owner Gates, plan-only router");