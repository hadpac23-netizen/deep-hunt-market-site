(() => {
  ...
  function answerVerified(){
    const p=state?.product||{}, retail=state?.retail||{}, variants=state?.variants||[];
    const parts=[];
    ...
    if(quotePass)parts.push("The product quote is marked verified.");
    else parts.push("Stock and shipping still require a fresh checkout recheck.");
    if(p.availability_verified===true)parts.push("Supplier availability has a verified signal.");
  ...
  function answerCompatibility(){
    ...
    const parts=[];
    if(p.model)parts.push(`Provider model: ${clean(p.model,120)}.`);
    ...
    if(!parts.length)return "HUNT does not currently have enough verified model, size, or compatibility data to promise a fit. Check the exact product/variant details before checkout.";
