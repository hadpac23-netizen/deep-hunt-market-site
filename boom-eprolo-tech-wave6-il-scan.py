...
CANDS=[
 {"category":"phone-stand","product_id":"26971060","supplier_category_id":31},
 {"category":"cable-organizer","product_id":"26921252","supplier_category_id":45},
 {"category":"sata-usb-adapter","product_id":"26940108","supplier_category_id":45},
 {"category":"tablet-stand","product_id":"26965037","supplier_category_id":31},
 {"category":"usb-hub","product_id":"26972478","supplier_category_id":126},
 {"category":"docking-hub","product_id":"26972449","supplier_category_id":126},
 {"category":"usb-card-reader-hub","product_id":"24939686","supplier_category_id":126},
 {"category":"dual-hdmi-hub","product_id":"26972443","supplier_category_id":126},
]
BLOCKED=re.compile(r"\b(weapon|knife|blade|gun|laser|vape|nicotine|medical|therapy|health|apple|iphone|ipad|samsung|xiaomi|huawei|sennheiser|whoop|garmin|fitbit|nintendo|ps5|xbox)\b",re.I)
def creds():
 vals={}
...
def signed_get(path,params):
 ...
 req=urllib.request.Request(BASE+path+"?"+urllib.parse.urlencode(q),headers={"apiKey":key,"Accept":"application/json","User-Agent":"HUNT-TECH-WAVE6/1.0"})
...
for i,c in enumerate(CANDS,1):
 ...
 try:
  ...
  picked=None
  for cost,w,ns,vid,v in variants[:8]:
   try:
    ...
    if st==200 and str(b.get("code"))=="0" and sh:
     ...
     spec="PASSIVE_ACCESSORY" if c["category"] in {"phone-stand","cable-organizer","tablet-stand"} else "TECHNICAL_SPEC_REVIEW"
  ...
  results.append(out);print("DONE",json.dumps({"id":c["product_id"],"status":out["status"],"ratio":out.get("shipping_to_retail_ratio"),"flag":out.get("commercial_training_flag"),"spec":out.get("spec_gate")}),flush=True)
...
out={"version":"HUNT-EPROLO-TECH-WAVE6-IL-SCAN-V1","date":"2026-09-23","mode":"READ_ONLY_SHADOW","destination":"IL","production_effect":False,"summary":summary,"results":results}
(ROOT/"evidence/HUNT-EPROLO-TECH-WAVE6-IL-SCAN-2026-09-23.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n")
