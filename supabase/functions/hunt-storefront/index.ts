...
function cjRetailPrice(sourceCost: number) {
  ...
  const marginFloor = cost / Math.max(0.05, reserveDenominator - targetMargin);
...
async function cjMarketShelves(focusSlug = "") {
  ...
  const focusedQueries: Record<string, string[]> = {
    petclothing: ["dog clothing", "cat clothing", "pet sweater", "pet coat"],
    aquarium: ["aquarium accessories", "fish tank accessories", "aquarium filter", "aquarium decoration"],
    mensleepwear: ["men pajamas", "men sleepwear", "men pajama set", "men lounge sleepwear"],
    menaccessories: ["men belt", "men cap", "men scarf", "men fashion accessories"],
    kidsshoes: ["kids shoes", "boys sneakers", "girls shoes", "children sandals"],
    babyshoes: ["baby shoes", "toddler first walker shoes", "infant shoes", "baby sneakers"],
    activebottoms: ["men gym shorts", "women workout leggings", "sports shorts", "active pants"],
    ...
    kidsunderwear: ["kids cotton underwear", "boys boxer briefs"],
    menunderwear: ["men cotton boxer briefs", "men underwear"],
    maternity: ["women maternity dress", "maternity clothing"],
    loungewear: ["women loungewear set", "women lounge set"],
    petite: ["women petite dress", "petite clothing"],
    plussize: ["women plus size dress", "plus size clothing"],
    suits: ["men formal suit", "men blazer suit"],
    sets: ["women two piece set", "women clothing set", "women matching set", "women top pants set"],
    ...
    crafts: ["craft kit", "sewing craft supplies", "painting craft supplies"],
    gifts: ["gift set", "home gift decor", "desk gift"],
    hats: ["women fashion hat", "baseball cap", "beanie"],
    ornaments: ["home ornament", "decorative ornament"],
    wallart: ["wall art canvas", "home poster decor"],
    blankets: ["throw blanket", "soft home blanket"],
    bottoms: ["women trousers", "women jeans", "women casual pants"],
    drinkware: ["travel tumbler", "water bottle", "coffee mug"],
    hoodies: ["women hoodie", "men hoodie"],
    pillows: ["decorative pillow", "cushion pillow"],
    stationery: ["notebook planner", "journal stationery", "desk stationery"],
    toys: ["educational toy", "building block toy", "puzzle toy"],
    travel: ["travel organizer", "luggage organizer", "passport holder"],
    gaming: ["gaming keyboard", "gaming mouse pad", "game controller"],
    beauty: ["beauty mirror", "makeup organizer", "skincare tool"],
  ...
  const definitions: Record<string, RegExp> = {
    ...
    petclothing: /\b(pet|dog|cat)\b.*\b(clothing|clothes|sweater|coat|jacket|shirt|hoodie|dress)\b|\b(clothing|clothes|sweater|coat|jacket|shirt|hoodie|dress)\b.*\b(pet|dog|cat)\b/i,
    aquarium: /\b(aquarium|fish tank|fish bowl|aquatic)\b/i,
    mensleepwear: /\b(men(?:'s)?|man|male)\b.*\b(pajama|pyjama|sleepwear|nightwear|robe)\b|\b(pajama|pyjama|sleepwear|nightwear|robe)\b.*\b(men(?:'s)?|man|male)\b/i,
    menaccessories: /\b(men(?:'s)?|man|male)\b.*\b(belt|cap|hat|scarf|tie|accessor)\b|\b(belt|cap|hat|scarf|tie|accessor)\b.*\b(men(?:'s)?|man|male)\b/i,
    kidsshoes: /\b(kids?|children|boys?|girls?)\b.*\b(shoes?|sneakers?|sandals?|boots?)\b|\b(shoes?|sneakers?|sandals?|boots?)\b.*\b(kids?|children|boys?|girls?)\b/i,
    babyshoes: /\b(baby|infant|toddler)\b.*\b(shoes?|sneakers?|sandals?|boots?|first walker)\b|\b(shoes?|sneakers?|sandals?|boots?|first walker)\b.*\b(baby|infant|toddler)\b/i,
    activebottoms: /\b(gym|fitness|sports?|workout|running|yoga|active)\b.*\b(shorts?|pants|trousers|leggings|bottoms)\b|\b(shorts?|pants|trousers|leggings|bottoms)\b.*\b(gym|fitness|sports?|workout|running|yoga|active)\b/i,
    socks: /\b(sock|socks)\b/i,
    swimwear: /\b(swimwear|swimsuit|swimsuits|bikini|swim trunks)\b/i,
    kidsunderwear: /\b(kids?|children|child|boys?|girls?|youth)\b.*\b(underwear|briefs?|boxer briefs?|boxers?|underpants|undershirt)\b|\b(underwear|briefs?|boxer briefs?|boxers?|underpants|undershirt)\b.*\b(kids?|children|child|boys?|girls?|youth)\b/i,
    menunderwear: /\b(men(?:'s)?|man|male)\b.*\b(underwear|briefs?|boxer briefs?|boxers?|underpants|undershirt)\b|\b(underwear|briefs?|boxer briefs?|boxers?|underpants|undershirt)\b.*\b(men(?:'s)?|man|male)\b/i,
    maternity: /\b(maternity|pregnancy|pregnant|nursing wear|nursing dress|nursing top)\b/i,
    loungewear: /\b(loungewear|lounge set|lounge pants|lounge top|homewear|home wear)\b/i,
    petite: /\bpetite\b/i,
    ...
    sets: /\b(two[- ]?piece|2[- ]?piece|matching|co[- ]?ord|coord)\b.{0,70}\b(top|shirt|blouse|vest|hoodie|sweater|cardigan|jacket|dress|skirt|shorts|pants|trousers|pajama|outfit|clothing)\b.{0,40}\b(set|suit|outfit)\b|\b(top|shirt|blouse|vest|hoodie|sweater|cardigan|jacket)\b.{0,40}\b(and|&)\b.{0,20}\b(pants|trousers|shorts|skirt)\b.{0,30}\b(set|outfit)\b/i,
  ...
  const shelfNegativeRules: Record<string, RegExp> = {
    ...
    sets: /\b(gift set|bedding set|sheet set|duvet set|jewelry set|jewellery set|necklace|bracelet|earring|earrings|ring set|makeup set|cosmetic set|toy set|tool set|kitchen set|bath set|towel set|baby|toddler|kid|kids|child|children|boys?|girls?|pet|dog|cat)\b/i,
    ...
    loungewear: /\b(kids?|child|costume|cosplay|pet|dog|cat)\b/i,
    petite: /\b(kids?|child|doll|pet|dog|cat)\b/i,
    plussize: /\b(pet|dog|cat|costume|cosplay|shapewear|slimming|waist trainer)\b/i,
    suits: /\b(pet|dog|cat|swim|wetsuit|costume|cosplay)\b/i,
...
async function gootenMarketShelves() {
  ...
  const definitions: Record<string, RegExp> = {
    ...
    yoga:/\b(yoga mat|yoga mats)\b/i,
    sportstowels:/\b(yoga towel|yoga towels|sports towel|sport towel|gym towel|cooling towel)\b/i,
...
async function persistedCatalogShelves() {
  ...
  const { data, error } = await db
    ...
    .select("provider,item_id,category,title,image_url,price_amount,currency,price_basis,availability_verified,source_fresh_at,gender,stock_quantity,market_eligibility_status,last_stock_check_at")
    .eq("availability_verified", true)
    .order("source_fresh_at", { ascending: false })
    .limit(1500);
  if (error || !Array.isArray(data)) return {};

  const freeLaunchProviders = new Set(["CJdropshipping", "Printful", "Gooten"]);
  const out: Record<string, any[]> = {};
  for (const row of data) {
    const provider = cleanText(row?.provider);
    const itemId = cleanText(row?.item_id);
    const slug = cleanText(row?.category).toLowerCase();
    ...
    out[slug].push({
      ...
      price_basis: cleanText(row?.price_basis) || "HUNT_RETAIL_PROFIT_GATE",
      availability_verified: true,
...
function matterhornMarketShelves() {
  ...
  const adultOrLowFit = /\b(erotic|sexy|seductive|fetish|adult|bondage|thong|g[- ]?string|t[- ]?backs?)\b/i;
  for (const row of rows) {
    ...
    const gallery = Array.isArray(row?.gallery) ? row.gallery.filter((x:any)=>cleanText(x).startsWith("https://")) : [];
    const verifiedVariants = variants.filter((v:any)=>v?.availability_verified === true && Number(v?.stock_quantity || 0) > 0);
    const price = Number(row?.price_amount);

    // BOOM quality gate: real stock, usable media, real variants, brand, clean title and valid supplier price.
    if (
      ...
      adultOrLowFit.test(title + " " + subcategory) ||
      !allowedTitle(title + " " + subcategory)
    ) continue;
    ...
    const shelfItem = {
      ...
      variant_count: Number(row?.variant_count || verifiedVariants.length),
      size_data_source: "PROVIDER_VARIANTS",
      checkout_status: "MATTERHORN_ACCOUNT_TERMS_PENDING",
      catalog_discovery: true,
      merchant_product: true,
      quality_gate: "BOOM_PREMIUM",
      source_fresh_at: row?.source_fresh_at || null
    };
    if (!shelfItem.item_id) continue;
  ...
  const rank = (x:any) =>
    (x?.availability_verified === true ? 100 : 0) +
    Math.min(40, Number(x?.stock_quantity || 0)) +
    Math.min(20, Number(x?.variant_count || 0) * 2) +
    (Array.isArray(x?.gallery) ? Math.min(12, x.gallery.length * 3) : 0);
  for (const rows of Object.values(out)) rows.sort((a:any,b:any)=>rank(b)-rank(a));
  return out;
}
...
function mergeMarketShelves(...sources: Record<string, any[]>[]) {
  ...
  for (const source of sources) {
    for (const [slug, rows] of Object.entries(source || {})) {
      ...
      for (const row of Array.isArray(rows) ? rows : []) {
        ...
        const title = cleanText(row?.title);
        const image = cleanText(row?.image_url);
        if (!title || !allowedTitle(title) || !image.startsWith("https://")) continue;
        seen.add(key);
  ...
  const qualityScore = (row:any) => {
    let score = 0;
    if (row?.quality_gate === "BOOM_PREMIUM") score += 80;
    if (row?.availability_verified === true) score += 45;
    const price = Number(row?.retail_price_amount ?? row?.price_amount);
    ...
    if (cleanText(row?.brand)) score += 8;
    if (Array.isArray(row?.gallery) && row.gallery.length >= 2) score += 10;
    if (Number(row?.variant_count || 0) >= 2) score += 8;
    if (row?.provider === "Matterhorn Wholesale") score += 8;
    if (row?.provider === "Printful") score += 5;
    if (row?.provider === "Gooten") score += 5;
    return score;
  };

  for (const [slug, rows] of Object.entries(merged)) {
    rows.sort((a:any,b:any)=>qualityScore(b)-qualityScore(a));
    merged[slug] = rows.slice(0, 160);
  }
  return merged;
}
...
function providerState() {
  return [
    ...
    },
    {
      provider: "Gooten",
      state: "CATALOG_LIVE",
      connector_stage: env("GOOTEN_RECIPE_ID") ? "PUBLIC_CATALOG_LIVE_RECIPE_ID_PRESENT" : "PUBLIC_CATALOG_LIVE",
    },
    {
      provider: "Matterhorn Wholesale",
      state: "CATALOG_LIVE",
...
function checkoutMap() {
  ...
  const gootenActive = Boolean(
    env("GOOTEN_RECIPE_ID") &&
      env("GOOTEN_PARTNER_BILLING_KEY") &&
      enabled("GOOTEN_FULFILLMENT_APPROVED") &&
      enabled("HUNT_PAYMENT_PROVIDER_APPROVED")
  );
  ...
  return {
    ...
    Gelato: {
      mode: "APPROVAL_REQUIRED",
      note: "Public catalog is live in HUNT. Variants, shipping quotes and order fulfillment still require verified Gelato API credentials."
    },
    Prodigi: {
      mode: "APPROVAL_REQUIRED",
    ...
    Gooten: {
      mode: gootenActive ? "ONSITE_CAPABLE" : "APPROVAL_REQUIRED",
      note: gootenActive
...
Deno.serve(async (req: Request) => {
  ...
  if (url.searchParams.get("shelves") === "1") {
    const priorityCjPromise = focusShelf
      ? Promise.resolve({})
      : Promise.all([
          withProviderTimeout(cjMarketShelves("phonecases"), {}, 14000),
