...
CANDS=[
 {"department":"kitchen","category":"tableware","product_id":"31342874","supplier_category_id":112},
 ...
 {"department":"kitchen","category":"kitchen-textiles","product_id":"30370940","supplier_category_id":127},
 {"department":"tech","category":"phone-accessories","product_id":"31417827","supplier_category_id":45},
 {"department":"tech","category":"camera-accessories","product_id":"8532420","supplier_category_id":39},
 {"department":"tech","category":"card-reader","product_id":"31405053","supplier_category_id":45},
 {"department":"tech","category":"phone-cases","product_id":"24799417","supplier_category_id":36},
]
...
def signed_get(path,params):
 ...
 req=urllib.request.Request(BASE+path+"?"+urllib.parse.urlencode(q),headers={"apiKey":key,"Accept":"application/json","User-Agent":"HUNT-WAVE3B-PROGRESS/1.0"})
...
def product(cat,pid):
 for page in (1,2,3):
  ...
  data=b.get("data") or []
  r=next((x for x in data if str(x.get("product_id") or x.get("id"))==pid),None)
  if r:return r
...
for i,c in enumerate(CANDS,1):
 print(f"START {i}/{len(CANDS)} {c['product_id']} {c['department']}",flush=True)
 try:
  ...
  for cost,w,ns,vid,v in variants[:6]:
   try:
    ...
    sh=ship(b.get("data") if isinstance(b,dict) else {})
...
out={"version":"HUNT-EPROLO-WAVE3B-IL-SCAN-V1","date":"2026-09-22","mode":"READ_ONLY_SHADOW","destination":"IL","production_effect":False,"summary":summary,"results":results}
(ROOT/"evidence/HUNT-EPROLO-WAVE3B-IL-SCAN-2026-09-22.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n")
