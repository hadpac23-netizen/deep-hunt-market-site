(() => {
  const CART_KEY="hunt_deal_cart_v1";
  const ACTION_KEY="hunt_local_product_actions_v1";
  const STATE_KEY="hunt_supplier_gravity_v1";
  ...
  const cart=()=>{
    const rows=readJson(CART_KEY,[]);
    return Array.isArray(rows)?rows:[];
  };  function localActions(){
    const rows=readJson(ACTION_KEY,{});
    return rows&&typeof rows==="object"?rows:{};
  }
  function actionFor(item){
    const rawKey=clean(item?.provider)+":"+clean(item?.item_id);
    const normalizedKey=productKey(item);
    const local=localActions()[rawKey]||localActions()[normalizedKey]||{};
    const remote=accountActions.get(normalizedKey)||accountActions.get(rawKey)||{};
    return {
      liked:Boolean(local.liked||remote.liked),
      saved:Boolean(local.saved||remote.saved),
      category:clean(local.category||remote.category||item?.category)
    };
  }
  function pinned(item){
    const row=actionFor(item);
    return row.liked||row.saved;
  }
  function state(){
    const row=readJson(STATE_KEY,{});
    return row&&typeof row==="object"?row:{};
  }
  function saveState(next){writeJson(STATE_KEY,next);return next;}
  ...
  function cartProviders(rows=cart()){
    ...
    if(!providers.has(current)){
      next.anchor_provider=[...providers.values()].sort((a,b)=>a.first-b.first)[0]?.provider||null;
    }else next.anchor_provider=current;
  ...
  function affinityProvider(){
    const rows=Object.entries(state().affinity||{})
      .map(([provider,row])=>({provider,score:Number(row?.score||0),last:Number(row?.last_seen||0)}))
  ...
  function rankCategories(keys,{inventory={}}={}){
    ...
    const rows=[...Object.values(localActions()),...accountActions.values()];
    const pinnedByCategory=new Map();
    rows.forEach(row=>{
      if(!(row?.liked||row?.saved))return;
    ...
    return (Array.isArray(keys)?keys:[]).filter(key=>{
      if(!Object.prototype.hasOwnProperty.call(inventory,key))return true;
      return Number(inventory[key]||0)>0||Number(pinnedByCategory.get(key)||0)>0;
    }).sort((a,b)=>{
      const score=key=>Number(pinnedByCategory.get(key)||0)*1000+Number(signals[key]||0)*10+
        Math.log1p(Math.max(0,Number(inventory[key]||0)))*4;
      return score(b)-score(a);
    });
  }  function evaluateSecondary({
    candidateProvider,cartRetailAmount=0,contributionBeforeExtraShipping=0,
    incrementalShipping=0,cartItemCount=0,explicitSearch=false,
    minContribution=1.5,minMarginRate=0.12,maxAutoCustomerShipping=1.99
  }={}){
    const ctx=context();
    const candidate=providerKey(candidateProvider);
    if(!ctx.anchor_provider||candidate===ctx.anchor_provider){
      return {provider:candidate,anchor_provider:ctx.anchor_provider,auto_open:true,
        requires_user_choice:false,hunt_shipping_subsidy:0,customer_shipping_increment:0,
        advisory_only:true};
    }
    ...
    return {
      provider:candidate,anchor_provider:ctx.anchor_provider,
      auto_open:autoOpen,requires_user_choice:!autoOpen,
  ...
  window.addEventListener("storage",event=>{
    if(event.key===CART_KEY)syncAnchor();
  });
