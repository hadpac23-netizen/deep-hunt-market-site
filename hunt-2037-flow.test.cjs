...
assert(js.includes("REAL CITY PHOTO · LICENSED"),"visible licensed World photo label missing");
...
assert(visual.includes("REAL NIGHT"),"real-photo city truth label missing");
...
assert(visual.includes("startRotation"),"real-photo city rotation missing");
...
assert.equal(cities.default_city,"tokyo","Tokyo must be the default opening city");
...
assert.equal(Core.WORLDS.find(x=>x.id==="fashion").visual,"assets/hunt-city/real/tokyo.jpg");
assert.equal(Core.WORLDS.find(x=>x.id==="jewelry").visual,"assets/hunt-city/real/paris.jpg");
assert.equal(Core.WORLDS.find(x=>x.id==="tech-home").visual,"assets/hunt-city/real/shenzhen.jpg");
assert.equal(Core.WORLDS.find(x=>x.id==="travel").visual,"assets/hunt-city/real/dubai.jpg");
for(const id of ["tokyo","paris","shenzhen","dubai"]){
  const city=cities.cities.find(x=>x.id===id);
  assert.equal(city.status,"LICENSED_PHOTO_READY","licensed city asset status missing for "+id);
  assert.equal(city.asset_kind,"licensed_photo","licensed city asset kind missing for "+id);
  assert.ok(String(city.asset_url||"").startsWith("assets/hunt-city/real/"),"real city photo path missing for "+id);
  assert(fs.existsSync(city.asset_url),"city photo missing for "+id);
  assert(fs.statSync(city.asset_url).size>100000,"city photo too small for "+id);
}
