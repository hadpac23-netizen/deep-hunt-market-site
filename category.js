(() => {
  ...
  async function loadMore() {
    ...
    try{
      ...
      const res=await fetch(pagePath+"?v=taxonomy4",{cache:"force-cache"});
  ...
  async function load() {
    ...
    try {
      const manifestRes = await fetch("catalog-manifest.json?v=taxonomy4",{cache:"force-cache"});
      if(manifestRes.ok){
        ...
        if(catalogPages.length){
          ...
          const firstRes=await fetch(firstPath+"?v=taxonomy4",{cache:"force-cache"});
          if(firstRes.ok){
