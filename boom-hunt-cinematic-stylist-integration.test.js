...
assert.equal(c.current_state.full_shelves_shadow_preview,"PASS_17_DEPARTMENTS_131_RAILS");
assert.equal(c.current_state.full_shelves_category_rails,131);
assert.equal(c.current_state.full_shelves_unique_products,5179);
assert.equal(c.current_state.full_shelves_initial_cards,1676);
...
assert.equal(c.current_state.cj_verified_4_market_products,36);
assert.equal(c.current_state.verified_truth_products_available,51);
assert.equal(c.current_state.original_zero_categories,38);
assert.equal(c.current_state.cj_filled_zero_categories,36);
assert.equal(c.current_state.remaining_zero_categories,2);
assert.deepEqual(c.current_state.remaining_zero_category_keys,["home/laundry","home/entryway"]);
...
assert.equal(academy.current_state.full_shelves_shadow_preview,"PASS_17_DEPARTMENTS_131_CATEGORY_RAILS");
assert.equal(academy.current_state.full_shelves_unique_products,5179);
assert.equal(academy.current_state.full_shelves_taxonomy_discipline,"PASS_NO_CROSS_DEPARTMENT_DUPLICATION");
assert.equal(academy.current_state.cj_verified_4_market_products,36);
assert.equal(academy.current_state.remaining_zero_categories,2);
